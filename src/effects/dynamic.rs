...
use crate::utils::{color_blend, fill, next_rand, sine8};

/// All pixels a single random color, changing each call.
pub fn random_color<P: Pixel>(pixels: &mut [P], state: &mut EffectState, params: &Params) {
...
pub fn single_dynamic<P: Pixel>(pixels: &mut [P], state: &mut EffectState, params: &Params) {
    if state.counter == 0 {
        let mut rng = state.aux;
        for p in pixels.iter_mut() {
            rng = next_rand(rng);
            *p = P::from_rgb8(params.wheel(rng as u8));
        }
        state.aux = rng;
    }
    ...
    pixels[idx] = P::from_rgb8(params.wheel(rng2 as u8));
    state.aux = rng2;
    state.counter = state.counter.wrapping_add(1);
}

/// Every pixel gets a new random wheel color each step.
pub fn multi_dynamic<P: Pixel>(pixels: &mut [P], state: &mut EffectState, params: &Params) {
    let mut rng = state.aux;
    for pixel in pixels.iter_mut() {
        rng = next_rand(rng);
        *pixel = P::from_rgb8(params.wheel(rng as u8));
    }
    state.aux = rng;
}
...
pub fn block_dissolve<P: Pixel>(pixels: &mut [P], state: &mut EffectState, params: &Params) {
    ...
    let target = |i: usize| {
        if color_idx == 0 {
            params.primary_at(i, len as usize)
        } else {
            params.colors[color_idx]
        }
    };
    ...
    if pixels[idx].to_rgb8() != target(idx) {
        pixels[idx] = P::from_rgb8(target(idx));
    ...
    for offset in 0..pixels.len() {
        ...
        if pixels[i].to_rgb8() != target(i) {
            pixels[i] = P::from_rgb8(target(i));
